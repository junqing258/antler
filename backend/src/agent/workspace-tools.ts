...
async function workspacePath(
  ...
  if (
    ...
    isAbsolute(relative(workspaceRoot, candidate))
  ) {
    throw new Error("Path must be inside the workspace.");
  }

  const existing = allowMissing
...
const readParameters = Type.Object({
  ...
  startLine: Type.Optional(
    Type.Integer({
      minimum: 1,
      description: "First file line to return (1-based).",
    }),
  ),
  endLine: Type.Optional(
    Type.Integer({
      minimum: 1,
      description: "Last file line to return (inclusive).",
    }),
  ),
});
...
const bashParameters = Type.Object({
  command: Type.String({
    minLength: 1,
    maxLength: 8_000,
    description: "Command to run with bash in the workspace.",
  }),
  ...
  const tools = [
    {
      ...
      label: "Read file",
      description:
        "Read a UTF-8 file or list a directory inside the workspace. Paths must be relative to the workspace.",
    ...
    },
    {
      ...
      label: "Edit file",
      description:
        "Replace one exact, unique text fragment in a UTF-8 workspace file.",
      ...
      async execute(_id: string, { path, oldText, newText }: EditInput) {
        ...
        if (content.indexOf(oldText, first + oldText.length) !== -1)
          throw new Error(
            "oldText occurs more than once; provide a unique fragment.",
          );
        await writeFile(
          target,
