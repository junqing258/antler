...
function parseProvider(value: unknown): ProviderRunConfig | undefined {
  ...
  if (
    ...
    !model.trim() ||
    (baseUrl !== undefined &&
      (typeof baseUrl !== "string" || (baseUrl && !URL.canParse(baseUrl))))
  )
    throw new Error(
      "供应商配置无效：请填写协议、API Key、模型和合法的 Base URL。",
    );
  return {
    protocol,
    apiKey: apiKey.trim(),
  ...
  app.post("/api/runs", async (request, reply) => {
    ...
    try {
      ...
      return reply.code(202).send({
        ...
        skillDiagnostics: outcome.skillDiagnostics,
      });
    } catch (error) {
      ...
      if (
        error instanceof
        (await import("../agent/host-runtime.js"))
          .ConversationSkillContextMismatchError
      )
        return reply
          .code(409)
          .send({ error: "conversation_skill_context_mismatch" });
      if (
        error instanceof
        (await import("../skills/skill-policy.js")).SkillPolicyError
      )
...
function parseKnowledgePolicy(
  value: unknown,
): import("../knowledge/types.js").KnowledgePolicy {
...
function parseSkillPolicy(
  ...
  if (policy.mode === "disabled" || policy.mode === "auto") {
    if (policy.skillIds !== undefined) throw new Error("Skill policy 无效。");
    return { mode: policy.mode as "disabled" | "auto" };
  }
  if (
    policy.mode === "selected" &&
    Array.isArray(policy.skillIds) &&
    policy.skillIds.every((x) => typeof x === "string" && x)
  ) {
    const skillIds = [...new Set(policy.skillIds)];
    if (skillIds.length > 16) throw new Error("Skill policy 无效。");
    return { mode: "selected" as const, skillIds };
  }
  throw new Error("Skill policy 无效。");
}
