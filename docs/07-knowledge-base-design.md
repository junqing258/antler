...
| 决策 | 本文档的方案 | 选择理由 |
|---|---|---|
| 向量/索引存储 | 单一 SQLite：MVP 使用 FTS5 倒排索引，向量能力经 `VectorIndex` 端口后置到 `sqlite-vec` | 后端已有 SQLite 权威存储，再引入第二个嵌入式数据库会增加打包、备份和一致性成本。`sqlite-vec` 不可用时可退化为纯 FTS5，不会阻塞 MVP |
| 检索接入方式 | run 前自动检索注入 + 固定引用；`search_knowledge` 工具列为后续触发条件（见第 13 节） | 纯工具模式会把“是否检索”的判断交给模型，存在不可控的漏检，并增加模型往返。自动注入可保证引用闭环和 `knowledge.retrieved` 审计事件对所有 run 一致可用；复杂任务确实需要按步骤主动检索时再增加 Agent 工具 |
| 检索策略起点 | MVP 仅使用 FTS5 关键词召回，hybrid 放到 v1 | 先验证增量索引、引用闭环与评测基线，将 embedding 的配置、限流和缓存复杂度后置；中文召回评测不达标是提前引入 embedding 的显式触发条件（见第 13 节） |

整体约束：检索文本不作为可信指令执行；SQLite 保存权威状态，索引可完整重建；embedding、检索引擎和解析器均通过自有接口隔离。
