...
function getText(message: Parameters<ChatModelAdapter['run']>[0]['messages'][number]) {
...
export function useAntlerRuntime(getServerInfo: () => Promise<ServerInfo>) {
  const adapter: ChatModelAdapter = {
    async *run({ messages, abortSignal }) {
      ...
      const headers = { 'content-type': 'application/json', 'x-antler-token': server.token };
      const created = await fetch(`${server.baseUrl}/api/tasks`, { method: 'POST', headers, body: JSON.stringify({ message }), signal: abortSignal });
      if (!created.ok) throw new Error('任务创建失败');
      const { eventsUrl } = await created.json() as { eventsUrl: string };
      const events = await fetch(`${server.baseUrl}${eventsUrl}?token=${encodeURIComponent(server.token)}`, { headers: { 'x-antler-token': server.token }, signal: abortSignal });
