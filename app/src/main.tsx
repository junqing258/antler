...
import { CircleHelpIcon, PencilIcon, PlusIcon, SettingsIcon, Trash2Icon, UserRoundIcon, XIcon } from 'lucide-react';
import './styles.css';
...
function SettingsDialog({ config, onSave, onClose }: { config: ProviderConfig; onSave: (config: ProviderConfig) => void; onClose: () => void }) {
  ...
  return <div className="settings-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="settings-dialog" aria-labelledby="settings-title" onMouseDown={(event) => event.stopPropagation()}>
      <aside className="settings-nav" aria-label="设置菜单">
        <h2 id="settings-title">设置</h2>
        <div className="settings-tabs" role="tablist" aria-orientation="vertical">
          {tabs.map(({ id, label, icon: Icon }) => <button key={id} type="button" role="tab" aria-selected={activeTab === id} className={activeTab === id ? 'is-active' : ''} onClick={() => setActiveTab(id)}><Icon aria-hidden="true" />{label}</button>)}
        </div>
        <button className="settings-close" type="button" onClick={onClose} aria-label="关闭设置"><XIcon /></button>
      </aside>
      <div className="settings-content">
        {activeTab === 'provider' && <form className="settings-form" aria-labelledby="provider-title" onSubmit={submit}>
          <div className="settings-heading"><div><h3 id="provider-title">供应商配置</h3><p>配置仅保存在当前浏览器的本地存储中。</p></div></div>
      ...
      <div className="settings-actions"><button type="button" onClick={() => setDraft(defaultProviderConfig)}>恢复默认</button><button className="settings-save" type="submit">保存配置</button></div>
        ...
        {activeTab === 'profile' && <section className="settings-panel" aria-labelledby="profile-title">
          <div className="settings-heading"><div><h3 id="profile-title">个人资料</h3><p>管理此设备上的个人信息。</p></div></div>
          <div className="profile-avatar">{displayName.slice(0, 1).toUpperCase() || 'U'}</div>
          <label>显示名称<input value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="输入你的名称" /></label>
          <p className="settings-note">个人资料当前仅保存在本次应用会话中。</p>
        </section>}
        {activeTab === 'about' && <section className="settings-panel about-panel" aria-labelledby="about-title">
          <div className="settings-heading"><div><h3 id="about-title">关于</h3><p>Antler 桌面助手</p></div></div>
          <div className="about-logo"><img src="/favicon.png" alt="Antler" /></div>
          <dl><div><dt>应用名称</dt><dd>Antler</dd></div><div><dt>当前版本</dt><dd>v0.1.0</dd></div><div><dt>运行环境</dt><dd>本地桌面应用</dd></div></dl>
        </section>}
      </div>
    </section>
  </div>;
